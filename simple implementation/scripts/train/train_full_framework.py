"""
Train the Full Framework (poster Fig. 3).

Pipeline: ResNet50 → IFPN → CD-DPA(P2-P4) → [Recon + RGR on P2] → Det head
...
Usage (from repo root):
    ...
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \\
        python -u scripts/train/train_full_framework.py 2>&1 | \\
        tee results/outputs_full_framework/train.log

For the 3-5 epoch sanity check, set num_epochs=5 in CONFIG.
"""
...
# ── Dataset (same dynamic import pattern as 14_train_cddpa.py) ────────
...
CONFIG = {
    ...
    'num_epochs':               50,      # set to 5 for sanity check
    'learning_rate':            1e-4,
    'backbone_lr':              1e-5,
    'weight_decay':             1e-4,
    'warmup_epochs':            3,
    ...
    'output_dir':               str(PROJECT_ROOT / 'results' / 'outputs_full_framework'),
...
def build_loaders():
    ...
    train_ds = VisDroneDataset(
        root, split='train', transforms=get_transforms(True)
    )
    val_ds = VisDroneDataset(
        root, split='val', transforms=get_transforms(False)
    )
...
def build_optimiser(model: nn.Module):
    """Separate LR for backbone vs head (same pattern as 14_train_cddpa.py)."""
    backbone_params, head_params = [], []
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        if 'base_model.backbone.body' in name:
...
def train_epoch(model, loader, optimiser, scheduler, scaler, epoch, writer):
    ...
    for step, (images, targets) in enumerate(
        ...
        for p in model.base_model.backbone.body.parameters():
...
def val_epoch(model, loader, epoch, writer):
    ...
    with torch.no_grad():
        for images, targets in tqdm(loader, desc=f"Val   E{epoch+1}", leave=False):
            images  = [img.to(DEVICE) for img in images]
            targets = [{k: v.to(DEVICE) for k, v in t.items()} for t in targets]
            with torch.cuda.amp.autocast(enabled=CONFIG['use_amp']):
                loss_dict = model(images, targets)
...
def main():
    ...
    parser.add_argument('--sanity', action='store_true',
                        help='Run 5-epoch sanity check then exit')
    args = parser.parse_args()
    if args.sanity:
        CONFIG['num_epochs'] = 5
        print("[Sanity] Running 5-epoch sanity check")
    else:
    ...
    scheduler = build_scheduler(optimiser, len(train_loader))
            ...
            for k, v in {**train_avg, **val_avg}.items():
                if not (isinstance(v, dict)):
                    assert torch.isfinite(torch.tensor(v)), \
                        f"Non-finite value in sanity check: {k}={v}"
