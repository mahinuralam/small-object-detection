"""
Evaluate FasterRCNN_FullFramework_DenseFPN on VisDrone val or test split.

Pipeline: ResNet50 → Standard FPN → DenseFPN → CD-DPA(P2-P4) → Recon+RGR → Det

Usage (from repo root):
    cd "simple implementation"

    # Val split (default):
    python scripts/eval/20_evaluate_full_framework_densefpn.py

    # Test split:
    python scripts/eval/20_evaluate_full_framework_densefpn.py --split test

    # Custom checkpoint:
    python scripts/eval/20_evaluate_full_framework_densefpn.py \
        --checkpoint results/outputs_full_framework_densefpn/best_model.pth
"""
...
def parse_args():
    default_ckpt = str(
        PROJECT_ROOT / 'results' / 'outputs_full_framework_densefpn' / 'best_model.pth'
    )
...
def main():
    ...
    print("=" * 70)
    print("FULL FRAMEWORK (DenseFPN) EVALUATION  —  poster Fig. 3")
    print("  ResNet50 → Standard FPN → DenseFPN → CD-DPA(P2-P4) → Recon+RGR → Det")
    print("=" * 70)
    ...
    print("\nLoading dataset...")
    dataset = VisDroneDataset(root_dir=args.data_root, split=args.split, transforms=None)
    loader  = DataLoader(
        dataset,
    ...
    if not ckpt_path.exists():
        raise FileNotFoundError(
            ...
            "Train first with:\n"
            "  torchrun --nproc_per_node=2 scripts/train/train_full_framework_densefpn.py"
        )
    ...
    print(f"  Loaded epoch {epoch_saved}  (val_loss={val_loss_saved:.4f})")
    ...
    baselines = {
        ...
        'CD-DPA only':                  43.44,
        'Full Framework (IFPN)':        None,   # fill in when available
    }
    for name, val in baselines.items():
        if val is None:
            ...
            delta = map_50 - val
            print(f"  {name:35s}: {val:5.2f}%  (DenseFPN Δ = {delta:+.2f}%)")
    print(f"  {'Full Framework (DenseFPN) — this':35s}: {map_50:5.2f}%")
    ...
    out = {
        ...
        'epoch':    epoch_saved,
        'mAP_50_95': round(map_50_95 / 100, 6),
        'mAP_50':    round(map_50    / 100, 6),
        'mAP_75':    round(map_75    / 100, 6),
    }
    if isinstance(per_class, torch.Tensor) and per_class.ndim > 0:
        ...
        out['per_class_AP_50_95'] = per_class_dict
